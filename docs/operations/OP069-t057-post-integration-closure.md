...
Contract-authoring PR: `#298`  
Durable receipt anchor: GitHub PR `#298`
...
The contract-authoring PR is `#298`. At execution time the Executor MUST:

1. read merged PR #298 from GitHub;
...
Do not hard-code a self-referential final branch SHA into this file; merged PR #298 is the deterministic authority.
...
- verify durable receipt publication capability to PR #298;
- verify PR #296 and PR #297 are merged into `develop`;
- verify PR #298 is merged into `develop`;
...
- publish exactly one final durable OP069 receipt to PR #298.
...
- PR #296, PR #297 and PR #298 confirmed merged into `develop`;
...
- final durable receipt successfully published to PR #298;
...
Publish one final top-level comment to PR #298 using exactly:
...
RECEIPT: https://github.com/ManuelBouza/agent-governance/pull/298
