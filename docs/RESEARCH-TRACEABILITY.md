...
| R015 | `docs/research/CHATGPT-LIBRARY-WORKTREE-SIMULATOR-RESEARCH.md` | COMPLETE | NOT_REQUIRED | R014; D058; `ManuelBouza/test_biblioteca` isolated target/topic branches; commits `c1208d6bb5f9e6cc28df7fcc4463d6144750cda9` and `1904904cde94c87bee17ab2e26d757d880f3fb07`; core lock collision tests; PR `test_biblioteca#3`; `docs/research/CHATGPT-LIBRARY-WORKTREE-SIMULATOR-LOCK-LIFECYCLE-APPENDIX.md`; Library snapshots under `/git-workspaces/ManuelBouza/test_biblioteca/worktree-simulator/` | none | The cross-chat worktree simulator core plus reusable lifecycle is empirically qualified and remains non-normative. One writable work unit maps to one topic branch, a dedicated persistent GitHub lock-branch namespace with a create/delete sentinel file as reusable ownership mutex, one portable standalone `.git` snapshot in a unique Library namespace, and ownership/freshness receipts checked before mutation. Two logical chats changed the same file independently; wrong-owner restore was blocked; PR #3 merged Chat A into an isolated target; the target snapshot was round-trip validated and promoted before Chat A Library GC; Chat A's sentinel was then released while Chat B remained active. A separate probe proved first sentinel creation wins, duplicate creation fails HTTP 422, exact-SHA deletion releases, and the same sentinel path can be reacquired. Ref-only acquisition remains evidence but is not the preferred reusable lifecycle because delete-ref was unavailable in the tested connector. Remaining gaps are a true wall-clock two-chat race, crash/orphan recovery, TTL/heartbeat, closed-unmerged cross-chat resume, and branch/ref retirement mechanics. No policy adoption is implied. |
...
R015 — ChatGPT Library worktree simulator
  ...
  remaining gaps: true simultaneous two-chat race, crash/orphan recovery, TTL/heartbeat, closed-unmerged cross-chat resume, branch/ref retirement mechanics
```
