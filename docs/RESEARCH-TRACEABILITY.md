...
| R029 | `docs/research/R029-AGENTS-SKILL-ARCHITECTURE-REFACTOR-RESEARCH.md` | COMPLETE | REJECTED | `docs/orchestrator/R029-S10-CANDIDATE-TOPOLOGY-AND-EVALUATION-PLAN.md`; `docs/orchestrator/R029-HUMAN-DISPOSITION.md` | none | Human Owner selected `REJECT_TOPOLOGY` on 2026-09-13, superseding the earlier recorded `ADOPT_FOR_DESIGN` disposition before any downstream design or implementation. The candidate lean-root/transverse-Skill topology is not adopted; the current architecture remains controlling. |

## Live research frontier

### R029 — AGENTS.md / Skill architecture refactor (closed)

```text
R029
  COMPLETE / REJECTED
  candidate topology: REJECTED
  current architecture retained: yes
  root AGENTS.md refactor: NOT AUTHORIZED
  new transverse Skill implementation: NOT AUTHORIZED
  normative decision adoption: none
  provider/model calls: 0
  scored observations: 0
  next gate: none; reconsideration requires a new explicit Human objective acknowledging this rejection
```

R029 remains historical research evidence only. Its S1-S10 analytical artifacts are preserved, but the synthesized topology was rejected by the Human Owner. No root/Skill materialization occurred, so the existing Agent Governance instruction and Maintainer Skill architecture remains controlling.
...
R027
  COMPLETE / EVALUATING
  predecessor responsibility-split evidence retained

R028
  COMPLETE / EVALUATING
  current Lean Executor analytical authority

D079
  ACCEPTED
  Lean Executor selected as architecture candidate for qualification
  qualification/adoption boundary is normative
  production ownership/routing migration: NOT ADOPTED
...
D079 requires T066 v2 as the fail-cheap screening gate and requires independent fresh-corpus confirmation before ownership-policy migration. Positive T066 screening evidence cannot directly promote R027/R028 into production policy.
...
Do not rewrite a completed research artifact merely to align with a later conclusion. Preserve the original analysis, create or reference the successor when material evidence changes, and use the registry to express supersession/current disposition.
