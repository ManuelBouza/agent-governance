...
| R017 | `docs/research/CHATGPT-LIBRARY-ONLY-DOCUMENT-GOVERNANCE-AND-CODEX-BRIDGE-RESEARCH.md` | COMPLETE | NOT_REQUIRED | official OpenAI Library/Codex/plugin/browser docs checked 2026-09-06; disposable Library-only overwrite/identity/duplicate-path experiment; Plugin Directory discovery; `docs/research/CHATGPT-LIBRARY-CODEX-BROWSER-BRIDGE-QUALIFICATION-APPENDIX.md`; blind Codex Desktop browser retrieval probe `R016-CODEX-BRIDGE-20260906` retained as historical fixture ID after R017 reallocation | none | Library-only documentary governance is feasible with application-level immutable content objects, SHA-256 identities, append-only receipts/promotions and explicit fork detection rather than Git-like in-place mutation. Strict Library-only multi-chat locking is not qualified because the tested surface exposed neither a usable CAS replacement primitive nor create-if-absent path semantics. No native first-party Codex Library mount/source was documented or discovered. The browser-mediated Library -> Codex local path is empirically verified for the tested surface: Codex reached authenticated Library, found the exact Library-only object, downloaded it without GitHub or receipt disclosure, and independently reproduced the undisclosed 261-byte size, SHA-256 and exact content. Codex -> Library immutable upload-back remains unqualified. No policy adoption is implied. |
...
R017 — Library-only documentary governance / Codex bridge
  COMPLETE / NOT_REQUIRED
  immutable Library objects + SHA-256 receipts + append-only promotions are the bounded design candidate
  ...
  Codex browser -> authenticated Library web -> exact download -> local SHA-256/content verification: EMPIRICALLY VERIFIED / VERSION_AND_SURFACE_SENSITIVE
