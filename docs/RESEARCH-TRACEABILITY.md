...
| R026 | `docs/research/R026-CHATGPT-GITHUB-INTERACTION-MINIMIZATION.md` | COMPLETE | EVALUATING | desk research 2026-09-12; empirical qualification pending on disposable repository | none | R026 recommends a read-local, bounded-publication, event-driven Orchestrator transport: prefer a persistent Work Desktop local repository when Git mechanics can be qualified; otherwise use an exact-ref local snapshot/cache plus Git Data tree/commit/ref batching. No D066 or bootstrap-policy change is adopted until an empirical interaction-budget pilot proves freshness, write safety and Human-free operation. |
