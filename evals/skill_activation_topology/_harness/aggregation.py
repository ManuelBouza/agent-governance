...
from .models import ACTIVATION_RELEVANT_CLASSES, NEGATIVE_CLASSES, FrozenInputs, HarnessError, TrialSpec
...
def conditional_third_specs(
    ...
    for case in ordered_cases(inputs):
        for candidate in allowed:
            ...
            if len(pair) == 3:
                continue
            split_terminal = candidate in {"F2", "G3"} and candidate_has_critical_violation(
                trials, candidate
            )
            if disagreement_fields(*pair) and not split_terminal:
...
def aggregate_candidate_trials(
    ...
    for case in inputs.corpus["cases"]:
        ...
        if len(repetitions) not in {2, 3}:
            raise HarnessError(f"{case['id']}--{candidate_id}: expected two or three valid repetitions")
        ...
        if not fields and len(repetitions) != 2:
            raise HarnessError(f"{case['id']}--{candidate_id}: forbidden unnecessary third repetition")
        ...
        aggregates.append(
            {
                ...
                "critical_violation_observed": any(_critical_violation(item) for item in repetitions),
            }
        )
    return aggregates
...
def qualification_futility_certificate(
    ...
    remaining_expected = sum(
        len(expected_entrypoints(inputs, TrialSpec(case, candidate_id, 1))) for case in remaining_cases
    )
    total_expected = sum(
...
def materiality_futility_certificate(
    ...
    """Conservative Regime-A helper retained for diagnostics; the v15 runner need not use it."""
