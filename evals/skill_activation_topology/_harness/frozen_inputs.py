...
def _validate_oracle_method(oracle: dict[str, Any]) -> None:
    if oracle.get("schema_version") != "12.0.0" or oracle.get("oracle_id") != (
        "MG1-T023-TOPOLOGY-ORACLE-v12"
    ):
        raise HarnessError("harness requires the frozen MG1 V12 oracle")
    ...
    if (
        ...
        raise HarnessError("oracle paired repetition/attempt method is not frozen V12")
    ...
    if (
        ...
        raise HarnessError("oracle v12 Windows workspace gate is incomplete")
...
def _validate_cases(inputs: FrozenInputs, known_capabilities: set[str]) -> None:
    ...
    if (
        inputs.corpus.get("schema_version") != "6.0.0"
        or not isinstance(cases, list)
        or len(cases) != 40
    ):
        raise HarnessError("harness requires the frozen 40-case MG1 V12 corpus")
    ...
    if set(fixtures) != {"neutral", "source", "consumer"}:
        raise HarnessError("trial-envelope fixture roles are not the frozen V12 set")
    for case in cases:
