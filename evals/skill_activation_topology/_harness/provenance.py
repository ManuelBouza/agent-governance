...
def _validate_schedule_records(
    ...
    if not method["global_base_valid_observations"] <= len(trials) <= method[
        "global_max_valid_observations"
    ]:
    ...
    for candidate in inputs.oracle["candidate_ids"]:
        ...
        if not method["per_candidate_base_valid_observations"] <= count <= method[
            "per_candidate_max_valid_observations"
        ]:
