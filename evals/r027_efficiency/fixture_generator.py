...
def render_arm(task:dict,d:Path)->None:
 token=task["variant_token"];arch=task["archetype"];acc=task["acceptance"]
 _w(d/"pyproject.toml",'[tool.pytest.ini_options]\npythonpath = ["."]\n');_w(d/"src/__init__.py","")
 if arch=="A":
  ...
  _w(d/"tests/test_acceptance.py",f"from src.subject import normalize_name\n\ndef test_required_normalization_delta():\n    assert normalize_name({acc['input']!r}) == {acc['expected']!r}\n\ndef test_preserved_normalization_behavior():\n    assert normalize_name({acc['preserved_input']!r}) == {acc['preserved_expected']!r}\n")
 elif arch=="B":
  ...
  _w(d/"tests/test_acceptance.py",f"from src.consumer import retry_budget\nfrom src.settings import SETTINGS\n\ndef test_required_config_sync_delta():\n    assert {acc['new_key']!r} in SETTINGS\n    assert {acc['old_key']!r} not in SETTINGS\n    assert retry_budget() == {acc['retry_budget']}\n")
 elif arch=="C":
  ...
  _w(d/"tests/test_acceptance.py",f"import pytest\nfrom src.cli import run\n\ndef test_required_version_delta():\n    assert run({acc['argv']!r}) == {acc['expected']!r}\n\ndef test_preserved_echo_behavior():\n    assert run({acc['preserved_argv']!r}) == {acc['preserved_expected']!r}\n\ndef test_preserved_unsupported_behavior():\n    with pytest.raises(ValueError):\n        run(['unknown'])\n")
