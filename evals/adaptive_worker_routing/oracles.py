"""Provider-free T063 Git/AST/fixture oracles and graders."""
...
def seed_p3_fixture(repo: Path, runtime_root: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if reproduction["returned_profile_name"] != "source-maintainer":
        raise HarnessError("P3 reproduction did not return source-maintainer")
    if reproduction["grants_source_maintenance"] is not True:
        raise HarnessError("P3 reproduction did not grant source maintenance")
    return fixture, {
...
def score_p3(value: dict[str, Any]) -> tuple[bool, str, int, int]:
    ...
    for finding in material:
        ...
        has_authority = (
            "authority" in combined
            or "permission" in combined
            or "grants_source_maintenance" in combined
            or "source maintenance" in combined
        )
