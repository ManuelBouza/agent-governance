...
2. append `start` and execute/verify the task;
3. resolve normal technical problems autonomously;
4. append `done` with evidence and verification when acceptance criteria are satisfied;
5. determine the next eligible task from WORKPLAN metadata and accumulated EXCHANGE state;
6. only after the prior task is DONE, load the next task record;
7. repeat until all authorized tasks are DONE or a valid blocker stops execution.
...
- no strategic/safety blocker exists.
...
Valid blocker reasons:
`missing_skill`, `conflicting_authority`, `scope_ambiguity`, `acceptance_ambiguity`, `production_risk`, `security_risk`, `destructive_change`, `external_dependency`, `human_decision_required`.
...
Task completion does not require an inter-task Strategy handoff. Continue immediately to the next eligible task.
