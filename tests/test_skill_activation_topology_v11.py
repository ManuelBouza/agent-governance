"""Mechanical identity and holdout-rotation checks for the T023 v11 epoch."""
...
def test_v11_identity_and_zero_prior_observation_policy(repo_root: Path) -> None:
    ...
    assert oracle["schema_version"] == "11.0.0"
    assert oracle["oracle_id"] == "MG1-T023-TOPOLOGY-ORACLE-v11"
    assert oracle["execution_epoch"] == "MG1-T023-EXECUTION-v11"
    assert corpus["schema_version"] == "5.0.0"
    assert corpus["corpus_id"] == "MG1-T023-CORPUS-v5"
    assert prior["mg1_v10_scored_observations"] == 0
    assert prior["prior_v2_v3_v4_v6_v7_v8_v9_v10_observations_may_enter_v11_score"] is False


def test_v5_differs_from_v4_only_by_wx01_to_wx00_rotation(repo_root: Path) -> None:
    ...
    assert set(current_cases) - set(previous_cases) == {"WX00"}
    assert set(previous_cases) - set(current_cases) == {"WX01"}
    for case_id in set(current_cases) & set(previous_cases):
        assert current_cases[case_id] == previous_cases[case_id]
    rotated = {
        key: value for key, value in current_cases["WX00"].items() if key not in {"id", "prompt"}
    }
    exposed = {
        key: value for key, value in previous_cases["WX01"].items() if key not in {"id", "prompt"}
    }
    assert rotated == exposed
    assert current_cases["WX00"]["prompt"] != previous_cases["WX01"]["prompt"]


def test_v10_exposed_prompt_is_absent_from_v11(repo_root: Path) -> None:
    corpus = json.loads((repo_root / CORPUS_PATH).read_text(encoding="utf-8"))
    attempt_path = repo_root / (
        "evals/skill_activation_topology/evidence/"
        "mg1-v10-codex-windows-gpt-5.6-sol-medium/attempts/WX01--B0--r1--a1.json"
    )
    attempt = json.loads(attempt_path.read_text(encoding="utf-8"))
    assert attempt["raw"]["prompt"] not in {case["prompt"] for case in corpus["cases"]}


def test_v11_preserves_frozen_presentations_topologies_and_envelope(repo_root: Path) -> None:
    change = _corpus_change(repo_root)
