...
def test_custom_agent_role_detection() -> None:
    ...
    assert m.custom_agent_role_keys(
        {
            "config": {
                "agents": {
                    "enabled": True,
...
def test_worker_receipt_exact() -> None:
    ...
    assert (
        m.validate_worker_receipt(value, spec=spec, task_digest=digest)["contract_nonce"]
        == spec.contract_nonce
    )
...
def test_tool_trace_detects_oracle_path() -> None:
    turn = {
        "items": [{"type": "commandExecution", "command": "Get-Content docs/reviews/T063-R6.md"}]
    }
...
def test_module_for_path_and_relative_import() -> None:
    ...
    assert o.resolve_import_targets("agent_governance.engine", node, scope) == {
        "agent_governance.source_adapter"
    }
